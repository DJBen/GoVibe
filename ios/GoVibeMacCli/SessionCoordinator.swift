...
final class SessionCoordinator {
    ...
    init(macDeviceId: String, pty: PtySession, logger: Logger, relayBase: String) {
    ...
    private func normalizeSnapshotLineEndings(_ payload: Data) -> Data {
        // tmux capture-pane emits LF-delimited rows; replaying raw LF can keep the
        // cursor column and produce diagonal rendering on TerminalView. Normalize
        // lone LF to CRLF for deterministic line starts during snapshot replay.
        var normalized = Data()
    ...
    private func specialProgramDisplayName(
        ...
        // Some Claude CLI launches surface only a version-like token (for example "2.1.17")
        // as pane_current_command; if any tty command line references Claude, prefer "Claude".
